"""2-D gauge-volume cross sections at multiple 2Θ angles."""
...
_pixel_pitch_mm = _fdr_params.detector().pixel_pitch / 1000.0  # mm
...
for angle, color in zip(angles, colors, strict=True):
    wedge = crystal_gague_shape(R, angle, offset=_pixel_pitch_mm * pixels / 2)
...
polygons = []
for angle in angles:
    wedge = crystal_gague_shape(R, angle, offset=_pixel_pitch_mm * 40 / 2)
...
ax.set_xlabel("z (mm)")
ax.set_ylabel("y (mm)")
_save(fig, "gauge_crosssection_fig2.png")
