...
_args = _fdr_params.CLIArgs(
    ...
    dpi=int(_ns.dpi),
    show=bool(_ns.show),
    energy_keV=_ns.energy_kev,
)
_gif_path = _args.outdir / "live_data_validation.gif" if _ns.gif else None
...
ax.set_xlim(-5, 55)
ax.legend(ncol=2)
_writer = PillowWriter(fps=1) if _gif_path else None

with contextlib.ExitStack() as _stack:
    if _writer is not None:
        _stack.enter_context(_writer.saving(fig, _gif_path, dpi=200))
